...
describe('per-launch structured feasibility', () => {
  it.each(['claude', 'codex', 'openclaude', 'grok', 'cursor'] as const)(
    'supports a local %s launch',
    (agent) => {
  ...
  const blockerCases: [string, Partial<StructuredNativeChatSupportInput>, string][] = [
    ...
    ['omp', { agent: 'omp' }, 'agent-without-structured-session'],
    ['gemini', { agent: 'gemini' }, 'agent-without-structured-session'],
