...
import { useStructuredAgentSessionProviderSwitch } from './use-structured-agent-session-provider-switch'
import {
...
export function useStructuredAgentSessionOptions(args: {
  ...
  const switchProvider = useStructuredAgentSessionProviderSwitch({
    mutate,
    optionSnapshot,
    transportEnabled,
    updateOptionState,
    ...
    async (id: string, value: string | boolean): Promise<boolean> => {
      ...
      const cross =
        id === 'model' && typeof value === 'string'
          ? isCrossProviderStructuredModelChoice(agent, value)
          : null
      if (cross && typeof value === 'string') {
        return switchProvider(cross, value)
      }
      const parsedModel =
        id === 'model' && typeof value === 'string' ? parseStructuredModelChoice(value) : null
      const optionValue: string | boolean = parsedModel?.modelId ?? value
      const encoded = encodeStructuredAgentSessionOptionValue(id, optionValue)
      if (
        !transportEnabled ||
        pendingOptionRef.current !== null ||
        !optionCatalog ||
        encoded === null ||
        !canSetStructuredAgentSessionOption(currentState, id, optionValue)
      ) {
        return false
      }
