...
export function createNativeChatMessageListProjection({
  preserveOrder = false
}: { preserveOrder?: boolean } = {}): (messages: NativeChatMessage[]) => NativeChatMessage[] {
  // A stable no-op sort keeps an order the caller already reconciled across transcript clocks.
  const compare = preserveOrder ? () => 0 : compareMessages
  let previous: NativeChatMessage[] = []
  let byId = new Map<string, NativeChatMessage>()
  return (messages) => {
    const folded = projectNativeChatTranscriptMessages(messages, compare)
