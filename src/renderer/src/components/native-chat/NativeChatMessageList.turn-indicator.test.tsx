...
describe('NativeChatMessageList turn indicator', () => {
  it('keeps the loading orb on the live row of a no-tool Codex turn', () => {
    render(
    ...
    const orb = row?.querySelector('.native-chat-loading-orb')
  ...
  it('keeps the live row distinct from the running tool row', () => {
    ...
    const orb = activity
      .closest('[data-native-chat-turn-activity]')
      ?.querySelector('.native-chat-loading-orb')
    expect(activity).not.toHaveClass('animate-pulse', 'animate-spin')
  ...
  // settles with the turn. Its motion is its own — the tail's orb never
  // migrates onto it — and both are gone once the turn is.
  it('keeps the trailing run live while the turn tail breathes, then settles both', () => {
    ...
    expect(liveRun?.querySelector('.native-chat-loading-orb')).toBeNull()
    ...
    expect(container.querySelector('.native-chat-loading-orb')).toBeNull()
  ...
  it('reads "Thinking" on the one live row while the turn is reasoning', () => {
    ...
    // One indicator, not a "Thinking" row stacked above a live "Working…" row.
