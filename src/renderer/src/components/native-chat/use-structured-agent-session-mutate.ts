...
export function useStructuredAgentSessionMutate(args: {
  ...
  const mutate = useCallback(
    async <T>(
      ...
      // A provider switch settles on the fence it created, so its own result still counts.
      if (
        !enabledRef.current ||
        (stateRef.current.fence !== targetFence && stateRef.current.fence !== result.fence)
      ) {
        return null
      }
