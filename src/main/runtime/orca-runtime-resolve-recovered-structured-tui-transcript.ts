...
export class OrcaRuntimeWithResolveRecoveredStructuredTuiTranscript extends OrcaRuntimeWithStopStructuredSessionProcess {
  ...
  protected async resolveStructuredAgentSessionIntent(
    ...
    // Adopting a provider conversation is only wired for the Claude and Codex transcript formats.
    const adoptableAgent = input.agent === 'claude' || input.agent === 'codex' ? input.agent : null
    const adoption =
      input.resumeFrom && adoptableAgent
        ? await resolveStructuredAgentSessionAdoptionForCreate({
            host,
            settings,
            agent: adoptableAgent,
