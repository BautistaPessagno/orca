...
async function install(deps: StructuredAgentSessionRuntimeDeps): Promise<InstalledRuntime> {
  ...
  const { resolveCodexEnvironment, resolveClaudeInheritedEnv, resolveAcpEnvironment } =
  ...
  try {
    ...
    // Serialized with teardown so a late exit cannot append after the journal directory is removed.
    const queueExitRecovery = (
      event: Parameters<StructuredAgentSessionHost['handleAdapterEvent']>[0]
    ) => {
    ...
    const claude = createStructuredClaudeRuntimeAdapter({
      ...
      ...(deps.getClaudeManagedAccountGateSettings
        ...
        : {}),
      onUnexpectedExit: queueExitRecovery,
      onBackgroundTasksChanged: (sessionId, state) =>
        host?.publishBackgroundTaskState(sessionId, state),
      onDispatchSettledLate,
    ...
    const acp = createAcpStructuredSessionAdapter({
      store,
      resolveWorkspacePath: deps.resolveWorkspacePath,
      resolveEnvironment: resolveAcpEnvironment,
      ...(deps.readProcessStartTime ? { readProcessStartTime: deps.readProcessStartTime } : {}),
